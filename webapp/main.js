...
async function pollState() {
  ...
  if (res.player.phase === "ended" && res.result) {
    ...
    const winningTeamName = res.result.winningTeamName || (winningTeam ? winningTeam.name : null);
    document.getElementById("result-subtitle").textContent = res.result.won
      ? `MVP: ${res.result.mvpName || "TBD"}`
      : `Winning Team: ${winningTeamName || (res.result.winningTeamId ? `Team #${res.result.winningTeamId}` : "Unknown")}`;
  }
}
