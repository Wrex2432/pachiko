...
function upsertPlayer(st, player) {
  ...
  const rawTeamIndex = Number.isInteger(player.teamIndex) ? player.teamIndex : 0;
  const teamIndex = Math.max(0, Math.min(FACECHINKO_TEAMS.length - 1, rawTeamIndex));
  st.playersByUid[uid] = {
    uid,
...
module.exports = {
  ...
  getWebPlayerState(session, uid) {
    ...
    return {
      ...
      color: team.color,
      winningTeamId: Number.isInteger(st.winningTeamIndex) ? st.winningTeamIndex + 1 : null,
      winningTeamName: Number.isInteger(st.winningTeamIndex) ? FACECHINKO_TEAMS[st.winningTeamIndex]?.name || null : null,
